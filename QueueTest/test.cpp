...
TEST(QueueTest, DequeueRemovesElements) {
    ...
    q.dequeue(); // должно удалить 1

    testing::internal::CaptureStdout();
    q.display();
    std::string output = testing::internal::GetCapturedStdout();

    EXPECT_EQ(output.find("1"), std::string::npos); // 1 больше нет
    EXPECT_NE(output.find("2"), std::string::npos);
    EXPECT_NE(output.find("3"), std::string::npos);
}

// Тест: dequeue на пустой очереди
TEST(QueueTest, DequeueOnEmptyQueueDoesNothing) {
    Queue q;
    EXPECT_NO_THROW(q.dequeue()); // просто проверим, что не падает
}

// Тест: несколько enqueue + dequeue
TEST(QueueTest, EnqueueDequeueMixed) {
    Queue q;
    q.enqueue(5);
    q.enqueue(6);
    q.dequeue();  // удалится 5
    q.enqueue(7);
